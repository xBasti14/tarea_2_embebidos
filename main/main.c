...
void serial_write(const char *data, int len) {
    int new_len = len + 1;
    char *data_with_end_of_message = malloc(sizeof(char) * new_len);
    memcpy(data_with_end_of_message, data, new_len);
    data_with_end_of_message[len] = '#';
    uart_write_bytes(UART_NUM_0, data_with_end_of_message, new_len);
}
...
void lectura() {
    ...
    while (1) {
        ...
        if ((tmp & 0b10000000) == 0x80) {
            ...
            int pos = 0;
            memcpy(data_to_send + pos, &acc_x, 2);
            pos += 2;
            memcpy(data_to_send + pos, &acc_y, 2);
            pos += 2;
            memcpy(data_to_send + pos, &acc_z, 2);
            pos += 2;
            memcpy(data_to_send + pos, &gyr_x, 2);
            pos += 2;
            memcpy(data_to_send + pos, &gyr_y, 2);
            pos += 2;
            memcpy(data_to_send + pos, &gyr_z, 2);
            pos += 2;
            serial_write(data_to_send, pos);
        }
    }
}

data_to_config wait_config() {
    char buf[5];
    data_to_config config;
    while (1) {
        int len = read_data(buf);

        if (len >= 5) {
            config.sensor = buf[0];
            config.acc_odr = buf[1];
            config.acc_range = buf[2];
            config.gyr_range = buf[3];
            config.gyr_odr = buf[4];
            break;
        }
    }
    return config;
...
void set_acc_conf(uint8_t val_acc_conf)
{
    //acc_odr values: 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c
    uint8_t reg_acc_conf=0x40;
    
    bmi_write(I2C_NUM_0, &reg_acc_conf, &val_acc_conf,1);
}

void set_acc_range(uint8_t acc_range)
{
    //acc_range values: 0x01, 0x02, 0x03
    uint8_t reg_acc_range=0x41;
    uint8_t val_acc_range=acc_range;
    
    bmi_write(I2C_NUM_0, &reg_acc_range, &val_acc_range,1);
}

void set_gyr_conf(uint8_t val_gyr_config)
{
    uint8_t reg_gyr_config=0x42;
    
    bmi_write(I2C_NUM_0, &reg_gyr_config, &val_gyr_config, 1);
}

void set_gyr_range(uint8_t val_gyr_range)
{
...
void config_bmi(data_to_config data){
    bmi_init();
    softreset();
    chipid();
    initialization();
    ...
    set_acc_conf(data.acc_odr);
    set_acc_range(data.acc_range);

    set_gyr_conf(data.gyr_odr);
    set_gyr_range(data.gyr_range);

    serial_write("STATUS200", 9);
